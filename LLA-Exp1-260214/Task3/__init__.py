...
class C(BaseConstants):
    ...
    # ── ECU exchange rate ────────────
    ECU_RATES = dict(
        en=dict(rate=0.50, currency='USD', symbol='$', example='10 ECU = $5.00'),
        fr=dict(rate=0.50, currency='EUR', symbol='€', example='10 ECU = €5.00'),
        de=dict(rate=0.50, currency='EUR', symbol='€', example='10 ECU = €5.00'),
        ja=dict(rate=75.0, currency='JPY', symbol='¥', example='10 ECU = ¥750'),
    )
...
class Player(BasePlayer):
    ...
    la2_endowment_ecu = models.FloatField(blank=True)
    ...
    la2_bonus_ecu = models.FloatField(blank=True)
    la2_bonus_real = models.FloatField(blank=True)
...
UI = dict(
    ...
    intro_text=dict(
        en=(
            ...
            'Amounts are shown in <strong>ECU (Experimental Currency Units)</strong>. '
            '<br><br>'
            'To start, you will be given <strong>{endowment} ECU</strong>. '
            ...
            'If you chose the safe option on that round, you simply keep your {endowment} ECU. '
            'If you chose the lottery, a coin flip decides whether you win more or lose some of your ECU. '
            '<br><br>'
            ...
            'Your final ECU total is converted at the rate shown below.'
        ),
        fr=(
            'Dans cette dernière partie, vous ferez une série de choix impliquant des gains et des pertes potentiels. '
            'Les montants sont indiqués en <strong>ECU (Experimental Currency Units)</strong>. '
            '<br><br>'
            'Pour commencer, vous recevrez <strong>{endowment} ECU</strong>. '
            ...
            'Si vous avez choisi l\'option sûre à ce tour, vous conservez simplement vos {endowment} ECU. '
            'Si vous avez choisi la loterie, un tirage au sort détermine si vous gagnez ou perdez des ECU. '
            '<br><br>'
            ...
            'Votre total final d\'ECU est converti au taux indiqué ci-dessous.'
        ),
        de=(
            'In diesem letzten Teil treffen Sie eine Reihe von Entscheidungen mit möglichen Gewinnen und Verlusten. '
            'Beträge werden in <strong>ECU (Experimental Currency Units)</strong> angezeigt. '
            '<br><br>'
            'Zu Beginn erhalten Sie <strong>{endowment} ECU</strong>. '
            ...
            'Wenn Sie in dieser Runde die sichere Option gewählt haben, behalten Sie einfach Ihre {endowment} ECU. '
            'Wenn Sie die Lotterie gewählt haben, entscheidet ein Münzwurf, ob Sie ECU gewinnen oder verlieren. '
            '<br><br>'
            ...
            'Ihr ECU-Gesamtbetrag wird zum unten angegebenen Kurs umgerechnet.'
        ),
        ja=(
            'この最終パートでは、利益と損失を含む一連の選択を行います。'
            '金額は<strong>ECU（Experimental Currency Units）</strong>で表示されます。'
            '<br><br>'
            'まず、<strong>{endowment} ECU</strong>が支給されます。'
            'その後、複数のラウンドでくじと安全な選択肢のどちらかを選びます。'
            '最後に、<strong>あなたの選択の中から1つがランダムに選ばれ、実際に実行されます。その結果は次の画面ですぐに発表されます</strong>。'
            'そのラウンドで安全な選択肢を選んでいた場合、{endowment} ECUをそのまま受け取ります。'
            'くじを選んでいた場合、コイン投げでECUが増えるか減るかが決まります。'
            '<br><br>'
            '<strong>支払い：</strong>調査完了後、<strong>参加者の10人に1人がランダムに選ばれ</strong>、課題ボーナスが<strong>Amazonギフトカード</strong>として支払われます。'
            '最終的なECU合計額は以下のレートで換算されます。'
        ),
    ),
    ...
    la2_intro=dict(
        en='You will now make a series of choices between a <strong>lottery</strong> and a <strong>safe option (0 ECU)</strong>. The lottery always gives you a 50% chance to <strong>win {gain} ECU</strong>. The potential loss changes with each choice. Your task is to find the loss amount where the lottery and the safe option feel <em>equally appealing</em> to you. <strong>Remember, the outcome of this task will be revealed to you immediately after you submit your decision.</strong>',
        fr='Vous allez maintenant faire une série de choix entre une <strong>loterie</strong> et une <strong>option sûre (0 ECU)</strong>. La loterie vous donne toujours 50% de chance de <strong>gagner {gain} ECU</strong>. Le montant de la perte change à chaque choix. Votre tâche est de trouver le montant de perte où la loterie et l\'option sûre vous semblent <em>aussi attractives l\'une que l\'autre</em>. <strong>N\'oubliez pas que le résultat de cette tâche vous sera révélé immédiatement après avoir soumis votre décision.</strong>',
        de='Sie treffen nun eine Reihe von Entscheidungen zwischen einer <strong>Lotterie</strong> und einer <strong>sicheren Option (0 ECU)</strong>. Die Lotterie bietet immer eine 50%-Chance, <strong>{gain} ECU zu gewinnen</strong>. Der mögliche Verlust ändert sich bei jeder Entscheidung. Ihre Aufgabe ist es, den Verlustbetrag zu finden, bei dem Lotterie und sichere Option für Sie <em>gleich attraktiv</em> sind. <strong>Denken Sie daran, dass Ihnen das Ergebnis dieser Aufgabe sofort nach dem Absenden Ihrer Entscheidung mitgeteilt wird.</strong>',
        ja='これから、<strong>くじ</strong>と<strong>安全な選択肢（0 ECU）</strong>の間で一連の選択を行います。くじは常に50%の確率で<strong>{gain} ECUを獲得</strong>できます。損失額は選択ごとに変わります。あなたの課題は、くじと安全な選択肢が<em>同じくらい魅力的に感じる</em>損失額を見つけることです。<strong>この課題の結果は、決定を送信した後すぐに発表されますのでご注意ください。</strong>'
    ),
    la2_step_label=dict(
    ...
    la2_sure_line=dict(
        en='<strong>0 ECU</strong> (keep what you have)',
        fr='<strong>0 ECU</strong> (garder ce que vous avez)',
        de='<strong>0 ECU</strong> (behalten, was Sie haben)',
        ja='<strong>0 ECU</strong>（今あるものをそのまま保持）'
    ),
    ...
    ecu_box_head=dict(
        en='Exchange Rate & Payment',
        fr='Taux de change & Paiement',
        de='Wechselkurs & Auszahlung',
        ja='換算レート＆支払い',
    ),
    ecu_box_note=dict(
        en='You start with {endowment} ECU. One of your choices will be played out for real. <strong>1 in 10 participants</strong> will be randomly selected to receive their bonus as an Amazon gift card at the rate above.',
        fr='Vous commencez avec {endowment} ECU. Un de vos choix sera joué pour de vrai. <strong>1 participant sur 10</strong> sera sélectionné au hasard pour recevoir son bonus sous forme de carte cadeau Amazon au taux ci-dessus.',
        de='Sie starten mit {endowment} ECU. Eine Ihrer Entscheidungen wird tatsächlich ausgespielt. <strong>1 von 10 Teilnehmenden</strong> wird zufällig ausgewählt, um den Bonus als Amazon-Gutschein zum obigen Kurs zu erhalten.',
        ja='{endowment} ECUからスタートします。あなたの選択の1つが実際に実行されます。<strong>10人に1人の参加者</strong>がランダムに選ばれ、上記のレートでAmazonギフトカードとしてボーナスが支払われます。',
    ),
    ...
    res_choice_a=dict(
        en='In this comparison, you chose the <strong>Safe Option</strong> (0 ECU).',
        fr='Dans cette comparaison, vous avez choisi l\'<strong>Option Sûre</strong> (0 ECU).',
        de='In diesem Vergleich haben Sie die <strong>Sichere Option</strong> (0 ECU) gewählt.',
        ja='この比較では、あなたは<strong>安全な選択肢</strong>（0 ECU）を選びました。'
    ),
    res_choice_b=dict(
    ...
    res_flip_win=dict(
        en='The coin flip resulted in a <strong>Win</strong>! You gained {gain} ECU.',
        fr='Le tirage au sort a donné une <strong>Victoire</strong> ! Vous avez gagné {gain} ECU.',
        de='Der Münzwurf ergab einen <strong>Gewinn</strong>! Sie haben {gain} ECU gewonnen.',
        ja='コイン投げの結果は<strong>勝ち</strong>でした！あなたは {gain} ECU を獲得しました。'
    ),
    res_flip_loss=dict(
        en='The coin flip resulted in a <strong>Loss</strong>. You lost {loss} ECU.',
        fr='Le tirage au sort a donné une <strong>Perte</strong>. Vous avez perdu {loss} ECU.',
        de='Der Münzwurf ergab einen <strong>Verlust</strong>. Sie haben {loss} ECU verloren.',
        ja='コイン投げの結果は<strong>負け</strong>でした。あなたは {loss} ECU を失いました。'
    ),
    res_final=dict(
        en='Your final balance for this task is: <strong>{endowment} ECU {sign} {outcome} ECU = {total} ECU</strong>.',
        fr='Votre solde final pour cette tâche est : <strong>{endowment} ECU {sign} {outcome} ECU = {total} ECU</strong>.',
        de='Ihr Endstand für diese Aufgabe ist: <strong>{endowment} ECU {sign} {outcome} ECU = {total} ECU</strong>.',
        ja='この課題の最終残高は：<strong>{endowment} ECU {sign} {outcome} ECU = {total} ECU</strong>。'
    ),
    res_final_a=dict(
        en='Your final balance for this task is your endowment: <strong>{total} ECU</strong>.',
        fr='Votre solde final pour cette tâche est votre dotation : <strong>{total} ECU</strong>.',
        de='Ihr Endstand für diese Aufgabe ist Ihr Startguthaben: <strong>{total} ECU</strong>.',
        ja='この課題の最終残高は初期残高のままです：<strong>{total} ECU</strong>。'
    ),
...
class Task3Intro(Page):
    ...
    def vars_for_template(player):
        ...
        ui = ui_dict(lang)
        endowment = int(C.LA_ENDOWMENT_ECU)

        ui['intro_text'] = ui['intro_text'].format(endowment=endowment)
        ui['ecu_box_note'] = ui['ecu_box_note'].format(endowment=endowment)
...
class LossAversionBisection(Page):
    ...
    def before_next_page(player, timeout_happened):
        lang = player.language
        ecu = C.ECU_RATES.get(lang, C.ECU_RATES['en'])
        endowment = C.LA_ENDOWMENT_ECU
        player.la2_endowment_ecu = endowment
        ...
        if steps:
            ...
            if selected['choice'] == 'A':
                ...
                player.la2_bonus_ecu = round(endowment, 2)
            else:
                if random.random() < 0.5:
                    player.la2_coin_flip = 'Win'
                    player.la2_bonus_ecu = round(endowment + C.GAIN_ECU, 2)
                else:
                    player.la2_coin_flip = 'Loss'
                    player.la2_bonus_ecu = round(endowment - selected['x_shown'], 2)
        else:
        ...
        player.la2_bonus_real = round(player.la2_bonus_ecu * ecu['rate'], 2)

        x_star = player.field_maybe_none('la2_indifference_x')
        if x_star is not None and x_star > 0:
            player.la2_lambda = round(C.GAIN_ECU / x_star, 4)
...
class LossAversionResult(Page):
    ...
    def vars_for_template(player):
        ...
        endowment = player.la2_endowment_ecu
        ...
        bonus_ecu = player.la2_bonus_ecu
        bonus_real = player.la2_bonus_real
        gain = C.GAIN_ECU
        ...
        res_flip_win = ui['res_flip_win'].format(gain=gain)
        res_flip_loss = ui['res_flip_loss'].format(loss=selected_x)
        res_final = ui['res_final'].format(endowment=endowment, sign=sign, outcome=outcome, total=bonus_ecu)
        res_final_a = ui['res_final_a'].format(total=bonus_ecu)
        ...
        return dict(
            ...
            symbol=C.ECU_RATES.get(lang, C.ECU_RATES['en'])['symbol'],
